...
describe('compareCron (Library)', () => {
    ...
    describe('C. Timezone Handling', () => {
        ...
        it('1) Timezone applies to both sides', () => {
            ...
            // They should differ because timezone offsets differ
            expect(utc.a.nextRuns[0]).not.toBe(ny.a.nextRuns[0]);
        });

        it('2) Invalid timezone throws', () => {
            expect(() => compareCron('@daily', '@hourly', 3, { timezone: 'Fake/Zone' }))
                .toThrow();
        });
    });
